...
将客服支持小组件集成到你的文档中。在你的 `docs.json` 文件中添加 `integrations` 字段，并填入相应的应用 ID 或聊天 ID。有关如何查找你的 ID 的说明，请参阅各项集成的文档。