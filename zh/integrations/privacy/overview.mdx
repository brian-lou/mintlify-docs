...
如果你需要出于 GDPR 合规考虑检查用户是否已同意使用 Cookie，可以在 `cookies` 下指定本地存储的键和值：
...
如果设置了这些值，你的站点会检查本地存储中指定的键和值，以判断用户是否已同意使用 cookie。如果用户尚未同意使用 cookie，站点会为该用户禁用遥测。

如果你想为所有用户禁用遥测，请将以下内容添加到 `docs.json` 文件中：
...
<Note>
  如果您禁用了遥测，那么即使在仪表板中启用了反馈功能，也无法在文档页面收集反馈。
</Note>
