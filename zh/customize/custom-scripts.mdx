...
使用 Tailwind CSS v3 为 HTML 元素添加样式。你可以控制布局、间距、颜色以及其他视觉属性。常见的类包括：
...
不支持 Tailwind CSS 的任意值语法。对于自定义数值，请改用 `style` 属性。
...
<Warning>
  使用 `style` prop 可能会在页面加载时导致布局偏移，尤其是在自定义模式的页面上。请改用 Tailwind CSS 类或自定义 CSS 文件，以避免页面布局跳动或闪烁。
</Warning>
...
将 CSS 文件添加到你的仓库后，其中定义的类名将在所有 MDX 文件中生效并可供使用。

<div id="adding-stylecss">
  ### 添加 `style.css`
</div>

例如，你可以添加一个名为 `style.css` 的文件，用来自定义导航栏和页脚的样式。
...
Mintlify 提供了一组常用的标识符和选择器，帮助你标记 UI 中的重要元素。

<Tip>
  使用“检查元素”来找到你想要自定义的相关元素引用。
</Tip>
...
<Warning>
  随着平台不断演进，引用内容和常见元素的样式可能会发生变化。请谨慎使用自定义样式。
</Warning>
...
自定义 JS 允许你在全局添加自定义可执行代码，相当于在每个页面中加入一个包含 JS 代码的 `<script>` 标签。

<div id="adding-custom-javascript">
  ### 添加自定义 JavaScript
</div>

文档内容目录中的任何 `.js` 文件都会自动被引入到每个文档页面中。例如，你可以添加如下所示的 `ga.js` 文件，在整个文档中启用 [Google Analytics](https://marketingplatform.google.com/about/analytics)。
