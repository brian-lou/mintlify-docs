---
title: "折叠面板"
description: "使用折叠面板来显示或隐藏内容。"
keywords: ["折叠面板", "可折叠内容", "可展开区域", "渐进式呈现"]
---

import IconsOptional from "/snippets/zh/icons-optional.mdx";

折叠面板使用户能够展开和收起内容区域。使用折叠面板进行渐进式信息展示，并帮助组织信息。


<div id="single-accordion">
  ## 单个手风琴组件
</div>

<Accordion title="我是一个手风琴组件（Accordion）。">
  你可以在这里放置任何内容，包括其他组件，比如代码：

  ```java HelloWorld.java
   class HelloWorld {
       public static void main(String[] args) {
           System.out.println("Hello, World!");
       }
   }
  ```
</Accordion>

````mdx Accordion example
<Accordion title="我是一个折叠面板。">
  您可以在此处放置任何内容，包括其他组件，例如代码：

   ```java HelloWorld.java
    class HelloWorld {
        public static void main(String[] args) {
            System.out.println("Hello, World!");
        }
    }
  ```
</Accordion>
````


<div id="accordion-groups">
  ## 手风琴分组
</div>

使用 `<AccordionGroup>` 将相关的手风琴组合在一起。这样可以创建一个统一的手风琴分组，每个手风琴都可以单独展开或收起。

<AccordionGroup>
  <Accordion title="入门">
    你可以在手风琴中嵌套其他组件。

    ```java HelloWorld.java
    class HelloWorld {
        public static void main(String[] args) {
            System.out.println("Hello, World!");
        }
    }
    ```
  </Accordion>

  <Accordion title="高级功能" icon="bot">
    添加图标可以让手风琴在视觉上更易区分并便于快速浏览。
  </Accordion>

  <Accordion title="故障排除">
    将相关内容按组进行组织管理。
  </Accordion>
</AccordionGroup>

````mdx Accordion Group Example
<AccordionGroup>
  <Accordion title="Getting started">
    You can put other components inside Accordions.

    ```java HelloWorld.java
    class HelloWorld {
        public static void main(String[] args) {
            System.out.println("Hello, World!");
        }
    }
    ```
  </Accordion>

  <Accordion title="Advanced features" icon="alien-8bit">
    Add icons to make accordions more visually distinct and scannable.
  </Accordion>

  <Accordion title="Troubleshooting">
    Keep related content organized into groups.
  </Accordion>
</AccordionGroup>
````


<div id="properties">
  ## 属性
</div>

<ResponseField name="title" type="string" required>
  在 Accordion 预览中显示的标题。
</ResponseField>

<ResponseField name="description" type="string">
  在 Accordion 预览中标题下方显示的详细描述。
</ResponseField>

<ResponseField name="defaultOpen" type="boolean" default="false">
  是否默认展开 Accordion。
</ResponseField>

<IconsOptional />