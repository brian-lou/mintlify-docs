...
title: "内容安全策略（CSP）配置"
sidebarTitle: "CSP 配置"
description: "配置 CSP 头部，在保持对反向代理、防火墙以及严格安全策略网络的安全性要求的同时，允许使用 Mintlify 资源。"
keywords: ["Content Security Policy", "CSP directives", "security headers", "XSS prevention"]
---

内容安全策略（CSP）是一项安全标准，通过控制网页可以加载哪些资源来帮助防止跨站脚本（XSS）攻击。Mintlify 提供了一个默认的 CSP，能够保护大多数站点。如果你的文档托管在会覆盖默认 CSP 的反向代理或防火墙之后，则可能需要配置 CSP 头部，以确保各项功能正常运行。

<div id="csp-directives">
  ## CSP 指令
</div>

以下 CSP 指令用于控制哪些资源可以加载：

- `script-src`: 控制允许执行的脚本
- `style-src`: 控制允许加载的样式表
- `font-src`: 控制允许加载的字体
- `img-src`: 控制允许加载的图片、图标和徽标
- `connect-src`: 控制可用于 API 调用和 WebSocket 连接的 URL
- `frame-src`: 控制允许嵌入到 frame 或 iframe 中的 URL
- `default-src`: 作为其他未显式设置指令的默认回退
...
| `*.mintlify.com` | 控制台、API、分析代理服务 | `connect-src` | 必需 |
...
| `cdn.jsdelivr.net` | 用于 OG 图片的表情资源 | `script-src`, `img-src` | 必需 |
| `mintlify.s3.us-west-1.amazonaws.com` | 托管在 S3 上的图片 | `img-src` | 必需 |
...
<Note>
  仅包含你实际使用的服务域名。删除任何尚未在你的文档中配置的分析服务域名。
</Note>
...
创建一个 Response Header Transform Rule（响应头转换规则）：
