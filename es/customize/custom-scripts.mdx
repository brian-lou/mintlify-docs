...
description: "Añade JavaScript y CSS personalizados para adaptar por completo el aspecto y la experiencia de tu documentación."
keywords: ["CSS", "JavaScript", "Tailwind CSS", "personalización de estilo"]
---

Usa CSS para aplicar estilos a los elementos HTML o añade CSS y JavaScript personalizados para adaptar por completo el aspecto y la experiencia de tu documentación.

<div id="styling-with-tailwind-css">
  ## Estilos con Tailwind CSS
</div>

Usa Tailwind CSS v3 para aplicar estilos a los elementos HTML. Puedes controlar el diseño, el espaciado, los colores y otras propiedades visuales. Algunas clases comunes son:

* `w-full` - Ancho completo
* `aspect-video` - Relación de aspecto 16:9
* `rounded-xl` - Esquinas muy redondeadas
* `block`, `hidden` - Control de visualización
* `dark:hidden`, `dark:block` - Visibilidad en modo oscuro

Los valores arbitrarios de Tailwind CSS no son compatibles. Para valores personalizados, usa la propiedad `style`.
...
<Warning>
  El uso de la prop `style` puede provocar un desplazamiento del diseño al cargar la página, especialmente en páginas en modo personalizado. Utiliza clases de Tailwind CSS o archivos CSS personalizados en su lugar para evitar estos desplazamientos o parpadeos.
</Warning>
...
Añade archivos CSS a tu repositorio y los nombres de clase que definas se aplicarán y estarán disponibles en todos tus archivos MDX.

<div id="adding-stylecss">
  ### Añadir `style.css`
</div>

Por ejemplo, puedes añadir el siguiente archivo `style.css` para personalizar el estilo de la barra de navegación y del pie de página.
...
Mintlify ofrece un conjunto de identificadores y selectores comunes para ayudarte a etiquetar elementos importantes de la interfaz de usuario.
...
<Warning>
  Las referencias y el estilo de los elementos comunes pueden cambiar a medida que la plataforma evoluciona. Usa los estilos personalizados con precaución.
</Warning>
...
El JavaScript personalizado te permite agregar código personalizado que se ejecutará de forma global. Es equivalente a añadir una etiqueta `<script>` con código JavaScript en cada página.

<div id="adding-custom-javascript">
  ### Agregar JavaScript personalizado
</div>

Cualquier archivo `.js` dentro del directorio de contenido de tu documentación se incluirá en todas las páginas de la documentación. Por ejemplo, puedes agregar el siguiente archivo `ga.js` para habilitar [Google Analytics](https://marketingplatform.google.com/about/analytics) en toda la documentación.
...
<Warning>
  Utilízalo con precaución para no introducir vulnerabilidades de seguridad.
</Warning>
