...
Los [componentes de React](https://react.dev) son una excelente manera de crear elementos interactivos y reutilizables en tu documentación.
...
En este ejemplo, se declara un componente `Counter` y luego se usa con `<Counter />`.
...
Para importar componentes de React en tus archivos MDX, los archivos de componentes deben estar en la carpeta `/snippets/`. Más información sobre los [Snippets reutilizables](/es/create/reusable-snippets).

<Note>
  No se admiten importaciones anidadas. Si un componente de React hace referencia a otros componentes, debes importar todos los componentes directamente en el archivo MDX principal, en lugar de importarlos dentro de los archivos de componentes.
</Note>
...
En este ejemplo se declara un componente `ColorGenerator` que usa varios hooks de React y luego se utiliza en un archivo MDX.
...
<AccordionGroup>
  <Accordion title="Impacto del renderizado en el cliente">
    Los componentes con hooks de React se renderizan en el cliente, lo que tiene varias implicaciones:

    * **SEO**: Es posible que los motores de búsqueda no indexen por completo el contenido dinámico.
    * **Carga inicial**: Los visitantes pueden ver brevemente un estado de carga antes de que se rendericen los componentes.
  ...
  <Accordion title="Prácticas recomendadas de rendimiento">
    ...
    * **Carga diferida**: Considera la carga diferida de componentes complejos para mejorar el tiempo de carga inicial de la página.
  </Accordion>
</AccordionGroup>