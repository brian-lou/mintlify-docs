...
Mintlify genera las siguientes metaetiquetas para cada página. Puedes sobrescribirlas si las especificas en tu `docs.json` o en el Frontmatter de una página.
...
* `og:image` - Imagen de Open Graph, con `og:image:width` establecido en 1200 y `og:image:height` en 630
* `twitter:image` - Imagen de Twitter, con `twitter:image:width` establecido en 1200 y `twitter:image:height` en 630

**Metadatos del navegador y de la aplicación:**
...
Cualquier metaetiqueta de la configuración `seo.metatags` de tu `docs.json` también se inyecta automáticamente en cada página, como `google-site-verification` para validar Search Console.

<div id="global-meta-tags">
  ## Metaetiquetas globales
</div>

Para configurar las metaetiquetas predeterminadas de todas las páginas, agrega el campo `metatags` a tu `docs.json`.
...
Se admiten las siguientes metaetiquetas a nivel de página:
...
* `og:image:height` - Alto de la imagen de Open Graph
...
* `twitter:image:height` - Alto de la imagen de Twitter
...
<Note>
  Las metaetiquetas con dos puntos deben ir entre comillas. El campo `keywords` debe tener el formato de una lista YAML.
</Note>
...
A continuación, encontrarás una lista completa de metaetiquetas que puedes añadir a tu `docs.json`. Estas metaetiquetas ayudan a mejorar el SEO de tu sitio, su apariencia al compartirse en redes sociales y la compatibilidad con los navegadores.

<Note>
  `og:image` añade una imagen de fondo sobre la que Mintlify superpone automáticamente tu logotipo, el título de la página y la descripción al generar las vistas previas para redes sociales.
</Note>

Puedes previsualizar cómo aparecerán tus metaetiquetas en distintas plataformas con [metatags.io](https://metatags.io/).
...
De forma predeterminada, solo se incluyen las páginas que aparecen en tu `docs.json`. Para incluir enlaces ocultos, agrega `seo.indexing` a tu `docs.json`:
...
Para añadir un archivo `sitemap.xml` o `robots.txt` personalizado, crea el archivo `sitemap.xml` o `robots.txt` en la raíz de tu proyecto. Al añadir un archivo personalizado, se sobrescribirá el archivo generado automáticamente con el mismo nombre. Si eliminas un archivo personalizado, se volverá a usar el archivo predeterminado.
...
También puedes especificar `noindex` para todas las páginas de tu documentación configurando el campo `metatags.robots` como `"noindex"` en tu `docs.json`:
...
<AccordionGroup>
  ...
  <Accordion title="Optimiza la estructura del contenido">
  ...
  <Accordion title="SEO de imágenes">
    ...
    * Optimiza el tamaño de los archivos de imagen para acelerar la carga
