...
<Info>
  El endpoint v1 para mensajes del asistente es compatible con **AI SDK v4**. Si usas AI SDK v5 o una versión posterior, usa en su lugar el [endpoint v2 para mensajes del asistente](/es/api-reference/assistant/create-assistant-message-v2).
</Info>
...
<Steps>
  ...
  <Step title="Usa el hook">
    ...
    * `streamProtocol: 'data'` - Obligatorio para transmitir respuestas en streaming.
    * `sendExtraMessageFields: true` - Obligatorio para enviar metadatos del mensaje.
