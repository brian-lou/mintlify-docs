...
description: "Configura los encabezados CSP para permitir recursos de Mintlify sin comprometer la seguridad en proxies inversos, firewalls y redes que aplican políticas de seguridad estrictas."
keywords: ["Política de seguridad de contenido", "directivas CSP", "encabezados de seguridad", "prevención de XSS"]
---

La Política de seguridad de contenido (CSP) es un estándar de seguridad que ayuda a prevenir ataques de cross-site scripting (XSS) al controlar qué recursos puede cargar una página web. Mintlify ofrece una CSP predeterminada que protege la mayoría de los sitios. Si alojas tu documentación detrás de un proxy inverso o firewall que sobrescribe la CSP predeterminada, es posible que debas configurar los encabezados CSP para que las funciones funcionen correctamente.
...
* `connect-src`: Controla a qué URL se pueden realizar conexiones para llamadas a la API y conexiones WebSocket
* `frame-src`: Controla qué URL se pueden incrustar en frames o iframes
* `default-src`: Se usa como valor predeterminado para otras directivas cuando no se establecen explícitamente
...
<Note>
  Incluye solo los dominios de los servicios que uses. Elimina cualquier dominio de analítica que no hayas configurado para tu documentación.
</Note>
...
1. En el panel de Cloudflare, ve a **Rules &gt; Overview**.
2. Selecciona **Create rule &gt; Response Header Transform Rule**.
3. Configura la regla:

* **Modify response header**: Establecer como estático
...
Identifica las infracciones de la CSP en la consola de tu navegador:
