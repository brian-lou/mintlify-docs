...
tag: "Obsolète"
keywords: [ "message de l'assistant", "intégrer", "chat", "intégration", "ai sdk v4" ]
---
...
<Steps>
  ...
  <Step title="Utiliser le hook">
    ...
    * `body.fp` - Identifiant d’empreinte (utiliser `anonymous` ou un identifiant utilisateur).
    ...
    * `body.context` - Tableau d’informations contextuelles à fournir à l’assistant. Chaque objet de contexte contient :
      ...
      * `value` - L’extrait de code ou le contenu de texte sélectionné.
      * `elementId` (optionnel) - Identifiant de l’élément d’interface contenant le contexte.
  </Step>
</Steps>

Consultez [useChat](https://ai-sdk.dev/docs/reference/ai-sdk-ui/use-chat) dans la documentation du SDK d’IA pour plus de détails.
