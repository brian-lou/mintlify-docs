...
- `anyOf` : accepte des données correspondant à n’importe lequel des schémas fournis. Fonctionne comme un opérateur `or`.
...
<Warning>Mintlify traite `oneOf` et `anyOf` de manière identique, car la différence pratique a rarement un impact sur l’utilisation de l’API.</Warning>
...
Lorsque vous utilisez `allOf`, Mintlify effectue un prétraitement de votre document OpenAPI pour afficher des combinaisons complexes de manière lisible. Par exemple, lorsque vous combinez deux schémas d’objet avec `allOf`, Mintlify regroupe les propriétés des deux dans un objet unique. Cela devient particulièrement utile lorsque vous exploitez les [components](https://swagger.io/docs/specification/components/) réutilisables d’OpenAPI.
...
Lorsque vous utilisez `oneOf` ou `anyOf`, les options sont affichées dans un conteneur à onglets. Indiquez un champ `title` dans chaque sous-schéma pour nommer vos options. Par exemple, voici comment vous pourriez afficher deux types différents d’adresses de livraison :
...
<ParamField body="delivery_address" type="object">
  <div className="mt-4 rounded-xl border border-gray-100 px-4 pb-4 pt-2 dark:border-white/10">
    <Tabs>
      <Tab title="AdressePostale">
        <ParamField body="address_line_1" type="string">
          L’adresse postale du domicile
        </ParamField>
      </Tab>

      <Tab title="BoîtePostale">
