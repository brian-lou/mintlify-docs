...
description: "Connectez des plateformes d’analyse pour suivre l’engagement des utilisateurs et l’utilisation de votre documentation."
keywords: ["outils d’analyse tiers", "suivi de l’engagement", "plateformes d’analyse", "suivi de l’utilisation"]
---

Suivez les interactions des utilisateurs avec votre documentation en connectant des plateformes d’analyse tierces. Mintlify envoie des événements d’engagement à vos services d’analyse configurés.

<div id="how-analytic-integrations-work">
  ## Comment fonctionnent les intégrations d’analyse
</div>

Lorsque vous ajoutez des intégrations d’analyse à votre site de documentation, Mintlify suit les interactions des utilisateurs, comme les pages vues, les requêtes de recherche, les requêtes effectuées dans le Bac à sable API et les envois de commentaires, puis les transmet à vos fournisseurs d’analyse. Vous pouvez connecter autant de fournisseurs d’analyse pris en charge que vous le souhaitez en ajoutant vos clés API au fichier `docs.json`. Les événements d’analyse sont envoyés à vos fournisseurs dès que vous les ajoutez à votre configuration, sans aucune configuration supplémentaire.
...
Ajoutez les identifiants de votre fournisseur d’analyse à l’objet `integrations` dans `docs.json`. Incluez uniquement les plateformes que vous souhaitez utiliser.

<Note>
  Les intégrations d’analyse nécessitent uniquement des clés d’API publiques, accessibles à toute personne qui consulte votre site de documentation. Vos clés d’API privées, associées aux clés publiques chez les fournisseurs d’analyse, doivent rester confidentielles. N’incluez pas de clés d’API privées dans votre `docs.json`.
</Note>
...
Tous les événements suivis utilisent le préfixe `docs.`.
...
| :-------------------------------------- | :------------------------------------------------------------------------------------------------------------------------------------ |
| `docs.content.view`                     | L’utilisateur consulte une page. Cet événement est envoyé uniquement aux fournisseurs qui ne suivent pas les vues de page par défaut. |
...
| Nom de l’événement         | Description                                        |
| :------------------------- | :------------------------------------------------- |
| `docs.search.query`        | L’utilisateur effectue une recherche.              |
...
<div id="code-and-api-playground">
  ### Code et bac à sable API
</div>

| Nom de l’événement            | Description                                                      |
| :---------------------------- | :--------------------------------------------------------------- |
| `docs.code_block.copy`        | L’utilisateur copie du code à partir d’un bloc de code.          |
| `docs.code_block.ask_ai`      | L’utilisateur demande à l’assistant d’expliquer un bloc de code. |
| `docs.api_playground.request` | L’utilisateur effectue une requête API dans le bac à sable API.  |
...
| `docs.expandable.open`  | L’utilisateur ouvre une section extensible. |
| `docs.expandable.close` | L’utilisateur ferme une section extensible. |
...
| `docs.assistant.completed`                | La session de chat se termine.                                                             |
| `docs.assistant.source_click`             | L&#39;utilisateur clique sur une référence dans une réponse du chat.                       |
...
| `docs.assistant.shared`                   | L&#39;utilisateur partage une conversation.                                                |
| `docs.assistant.thumbs_up`                | L&#39;utilisateur clique sur le bouton d’évaluation positive d’une réponse de l’assistant. |
| `docs.assistant.thumbs_down`              | L&#39;utilisateur clique sur le bouton d’évaluation négative d’une réponse de l’assistant. |
| `docs.assistant.spam_detected`            | L&#39;assistant détecte du spam dans un message utilisateur.                               |
...
| `docs.context_menu.ai_provider_click`  | L’utilisateur clique sur un fournisseur d’IA pour lancer une conversation avec la page actuelle comme contexte. |
| `docs.context_menu.install_mcp_server` | L’utilisateur installe le serveur MCP hébergé dans un éditeur de code.                                          |