---
title: "React"
description: "Créez des interfaces interactives et réutilisables avec des composants React."
...
Pour importer des composants React dans vos fichiers MDX, les fichiers des composants doivent être placés dans le dossier `/snippets/`. En savoir plus sur les [snippets réutilisables](/fr/create/reusable-snippets).

<Note>
  Les imports imbriqués ne sont pas pris en charge. Si un composant React fait référence à d&#39;autres composants, vous devez importer tous les composants directement dans le fichier MDX parent plutôt que d&#39;importer des composants dans les fichiers de composants eux‑mêmes.
</Note>
...
Cet exemple déclare un composant `ColorGenerator` qui utilise plusieurs hooks React, puis l’utilise ensuite dans un fichier MDX.
...
Importez le composant `ColorGenerator`, puis utilisez-le dans un fichier MDX :
...
<AccordionGroup>
  <Accordion title="Impact du rendu côté client">
    Les composants à hooks React sont rendus côté client, ce qui a plusieurs implications :

    * **SEO** : les moteurs de recherche pourraient ne pas indexer complètement le contenu dynamique.
    * **Chargement initial** : les visiteurs peuvent voir un bref flash de contenu en cours de chargement avant le rendu des composants.
    * **Accessibilité** : assurez-vous que les modifications de contenu dynamique sont annoncées aux lecteurs d&#39;écran.
  </Accordion>

  <Accordion title="Bonnes pratiques de performances">
    * **Optimiser les tableaux de dépendances** : incluez uniquement les dépendances nécessaires dans vos tableaux de dépendances `useEffect`.
    * **Mémoriser les calculs complexes** : utilisez `useMemo` ou `useCallback` pour les opérations coûteuses.
    * **Réduire les re-rendus** : divisez les gros composants en plus petits pour éviter les re-rendus en cascade.
