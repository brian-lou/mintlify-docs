...
* `rounded-xl` - Bords très arrondis
...
<Warning>
  L’utilisation de la prop `style` peut provoquer un décalage de mise en page au chargement de la page, en particulier sur les pages en mode personnalisé. Utilisez plutôt des classes Tailwind CSS ou des fichiers CSS personnalisés pour éviter les décalages ou les effets de scintillement.
</Warning>
...
Ajoutez des fichiers CSS à votre dépôt, et les noms de classe que vous y définissez seront appliqués et disponibles dans tous vos fichiers MDX.
...
Mintlify met à votre disposition un ensemble d’identifiants et de sélecteurs courants pour vous aider à repérer les éléments importants de l’interface utilisateur.

<Tip>
  Utilisez l’outil d’inspection pour trouver les références des éléments que vous souhaitez personnaliser.
</Tip>
...
<Warning>
  Les références et le style des éléments courants sont susceptibles de changer à mesure que la plateforme évolue. Utilisez les styles personnalisés avec prudence.
</Warning>
...
Le JavaScript personnalisé vous permet d’ajouter globalement du code exécutable personnalisé. Cela revient à ajouter une balise `<script>` contenant du code JS sur каждой page.

<div id="adding-custom-javascript">
  ### Ajouter du JavaScript personnalisé
</div>

Tout fichier `.js` placé dans le répertoire de contenu de votre documentation sera inclus sur каждой page de la documentation. Par exemple, vous pouvez ajouter le fichier `ga.js` suivant pour activer [Google Analytics](https://marketingplatform.google.com/about/analytics) sur l’ensemble de la documentation.
...
<Warning>
  Utilisez-le avec prudence afin de ne pas introduire de failles de sécurité.
</Warning>
