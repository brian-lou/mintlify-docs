...
description: "Ajoutez du JavaScript et du CSS personnalisés pour personnaliser entièrement l’apparence de votre documentation."
keywords: ["CSS", "JavaScript", "Tailwind CSS", "personnalisation du style"]
---

Utilisez CSS pour mettre en forme les éléments HTML, ou ajoutez du CSS et du JavaScript personnalisés afin de personnaliser entièrement l’apparence de votre documentation.

<div id="styling-with-tailwind-css">
  ## Mise en forme avec Tailwind CSS
</div>

Utilisez Tailwind CSS v3 pour appliquer des styles aux éléments HTML. Vous pouvez contrôler la mise en page, les espacements, les couleurs et d&#39;autres propriétés visuelles. Parmi les classes courantes :
...
Les valeurs arbitraires de Tailwind CSS ne sont pas prises en charge. Pour des valeurs personnalisées, utilisez la prop `style`.
...
<Warning>
  L’utilisation de la prop `style` peut entraîner un saut de mise en page au chargement de la page, notamment sur les pages en mode personnalisé. Utilisez plutôt des classes Tailwind CSS ou des fichiers CSS personnalisés pour éviter ces sauts ou tout effet de scintillement.
</Warning>
...
Ajoutez des fichiers CSS à votre dépôt et les noms de classes qu’ils définissent seront appliqués et disponibles dans l’ensemble de vos fichiers MDX.
...
Mintlify dispose d'un ensemble d'identifiants et de sélecteurs courants pour vous aider à marquer les éléments importants de l'interface utilisateur.

<Tip>
  Utilisez l'outil d'inspection d'éléments de votre navigateur pour trouver les références aux éléments que vous souhaitez personnaliser.
</Tip>
...
<Warning>
  Les références et la mise en forme des éléments courants sont susceptibles d’évoluer au fil des mises à jour de la plateforme. Veuillez utiliser les styles personnalisés avec précaution.
</Warning>
...
Le JavaScript personnalisé vous permet d’ajouter du code exécutable personnalisé de façon globale. C’est l’équivalent d’ajouter une balise `<script>` contenant du code JS dans chaque page.

<div id="adding-custom-javascript">
  ### Ajout de JavaScript personnalisé
</div>

Tout fichier `.js` placé dans le répertoire de contenu de votre documentation sera inclus dans chaque page de la documentation. Par exemple, vous pouvez ajouter le fichier `ga.js` suivant pour activer [Google Analytics](https://marketingplatform.google.com/about/analytics) sur l’ensemble de votre documentation.
