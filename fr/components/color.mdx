---
title: "Color"
description: "Affichez des palettes de couleurs avec une fonction de copie en un clic."
keywords: ["couleur", "palette", "couleurs", "design system", "hex", "rgb", "rgba", "hsl", "oklch"]
---

Utilisez le composant Color pour présenter des palettes de couleurs dans votre documentation. Les couleurs peuvent être affichées dans une grille compacte ou organisées dans un tableau avec des lignes libellées.

<div id="compact-variant">
  ## Variante compacte
</div>

Affichez les couleurs dans une grille simple avec leur nom et leur valeur.
...
Organisez les couleurs en lignes nommées pour la documentation du design system.
...
Définissez des couleurs distinctes pour les modes clair et sombre à l’aide d’un objet contenant les propriétés `light` et `dark`.
...
<ResponseField name="children" type="Color.Item | Color.Row" required>
  Éléments ou lignes de la palette de couleurs à afficher.
</ResponseField>
...
<ResponseField name="children" type="Color.Item" required>
  Couleurs à afficher dans la ligne.
</ResponseField>
...
<ResponseField name="value" type="string | { light: string, dark: string }" required>
  Valeur de la couleur dans n’importe quel format CSS, ou objet contenant des valeurs pour les modes clair et sombre.
</ResponseField>