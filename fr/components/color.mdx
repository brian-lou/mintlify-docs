---
title: "Couleur"
description: "Affichez des palettes de couleurs avec une fonction de copie au clic."
keywords: ["color", "palette", "colors", "design system", "hex", "rgb", "rgba", "hsl", "oklch"]
---

Utilisez le composant Color pour présenter des palettes de couleurs dans votre documentation. Les couleurs peuvent être affichées dans une grille compacte ou organisées dans une table avec des lignes libellées.

<div id="compact-variant">
  ## Variante compacte
</div>

Afficher les couleurs dans une grille simple avec leurs noms et valeurs.
...
Organisez les couleurs en lignes portant un libellé pour la documentation de votre système de design.

<Color variant="table">
  <Color.Row title="Principal">
...
Le composant prend en charge tous les formats de couleur pris en charge par CSS, notamment hex, rgb, rgba, hsl et oklch.
...
Définissez différentes couleurs pour les modes clair et sombre à l’aide d’un objet avec les propriétés `light` et `dark`.
...
<ResponseField name="variant" type="string" required>
  Style d'affichage de la palette de couleurs. Options possibles : `compact` ou `table`.
</ResponseField>
...
<ResponseField name="title" type="string">
  Libellé pour la rangée de couleurs.
</ResponseField>
...
<ResponseField name="value" type="string | { light: string, dark: string }" required>
  Valeur de couleur dans n'importe quel format CSS, ou objet avec des valeurs distinctes pour les modes clair et sombre.
</ResponseField>