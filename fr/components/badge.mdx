---
title: "Badge"
description: "Utilisez les badges pour mettre en avant un statut, un libellé ou des métadonnées, intégrés au texte ou affichés seuls."
keywords: ["badge", "étiquette", "tag", "statut", "indicateur"]
---

import IconsOptional from "/snippets/fr/icons-optional.mdx";

Utilisez des badges pour afficher des indicateurs d’état, des libellés ou des métadonnées. Les badges peuvent être utilisés au sein d’un texte ou comme éléments autonomes.
...
Les badges sont disponibles en quatre tailles pour refléter la hiérarchie de votre contenu.
...
Choisissez entre des badges à coins arrondis ou en forme de pilule.
...
Ajoutez des icônes aux badges pour donner davantage de contexte.
...
Utilisez la variante « contour » pour un rendu plus discret.
...
Désactiver les badges pour indiquer un état inactif ou indisponible.
...
Les badges s&#39;intègrent naturellement dans le contenu textuel. Par exemple, cette fonctionnalité nécessite un abonnement <Badge color="orange" size="sm">Premium</Badge>, et ce point de terminaison d&#39;API renvoie une réponse au format <Badge color="green" size="sm">JSON</Badge>.
...
<ResponseField name="stroke" type="boolean" default="false">
  Afficher le badge avec un contour au lieu d’un arrière-plan uni.
</ResponseField>
